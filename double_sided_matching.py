# Non Modular / non function wise implementation of the Double Sided Matching Algorithm
import os
import sys
import random
import numpy as np
...
#
def stable(manPref, manPrefIndex, womanPref):
...
def findRandomMarriagePrefIndex(randomMarriage):
    for i in range(int(n)):
        m = randomMarriage[i]
...
def plotGraph(manPrefIndex, manPrefIndexRandom):
    # Plotting Man Number Vs Woman Allocation Preference Index
    x = np.arange(int(n))

    # plt.plot(x,manPrefIndex,'b-',x,manPrefIndexRandom,'g--')
    plt.plot(x, manPrefIndex, "bo", label="Stable Matching")
    plt.plot(x, manPrefIndexRandom, "r^", label="Random Matching")

    plt.legend()
    plt.title("Random Matching Vs Stable Matching")

    plt.ylabel(
        "Preference Index of Allocated Service provider\n(High pref --> Low pref)"
    )
...
if __name__ == "__main__":
    print("\nStable Matching Allocation Algorithm Execution\n")
    ...
    marriage = []
    marriage = stable(manPref, manPrefIndex, womanPref)
    printMarriage(marriage)

    print("Man Preference Index In Stable Marriage : ", manPrefIndex)
    ...
    print("\n*******************************************************")
    print("\nExecuting Random Marriage Problem                      ")

    printMarriage(randomMarriage)
    findRandomMarriagePrefIndex(randomMarriage)
    print("Random Marriage Preference Index : ", manPrefIndexRandom)

    plotGraph(manPrefIndex, manPrefIndexRandom)
